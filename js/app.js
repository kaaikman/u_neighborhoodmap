  ...
  function NMViewModel() {
    ...
    function filterMarkers(type) {
      for (var cM = 0; cM < locDisplayedMarkers.length; cM++) {
        ...
        locDisplayedMarkers[cM].setVisible(false);
        // closes window so hidden marker info isn't left on screen
        locInfoWindow.close();
      }
      ...
      for (var dM = 0; dM < locDisplayedMarkers.length; dM++) {
        ...
        locDisplayedMarkers[dM].setVisible(true);
      }
      markerBounds();
    }
    ...
    function fourSquare(loc) {
      var fSLink = 'https://api.foursquare.com/v2/venues/' + loc.fsID + '?client_id=FQCEACSH5BBV2BQRYOW44DA0W02J2B40JLZRRMQ0V3XVMZ3I&client_secret=ZKBND1BGYZTNOI3EJH014BUE5VPQDGKSSE1PI2GRMYJJ5VKD&v=20161111';
      var fsRate;
      var fsLogo = 'img/fsLogo.png';
      $.getJSON(fSLink)
      .done(function(data, status) {
        // fsRate = data.response.venue.rating;
        !!data.response.venue.rating ? fsRate = data.response.venue.rating : fsRate = 'No rating available';
        ...
        !!fSImgC ? self.placesImg(fSImgC, fSImgSrc) : console.log('No image available - leaving default');
      })
      .fail(function() {
        alert("FourSquare data could not be loaded");
      });
    }
    self.placeImg = ko.observable('<img class="p-img" src="img/bath.jpg" title="Source: Pixfix"></img>');
    self.placesImg = function(pImg, pSrc) {
      self.placeImg('<img class="p-img" src="' + pImg + '" title="' + pSrc + '"></img>');
    };
